...
function sysCall_init()
    ...
    sim.addLog(sim.verbosity_scriptinfos,"This tool measures distance and direction between first clicked vertex/dummy and second clicked vertex/dummy.")
    sim.broadcastMsg{id='pointSampler.enable',data={key='measureDistanceDirection',vertex=true,dummy=true,snapToClosest=true}}
end
...
function sysCall_msg(event)
    ...
    if event.id=='pointSampler.click' then
        ...
        if not firstPoint then
            ...
            sim.broadcastMsg{id='pointSampler.setFlags',data={key='measureDistanceDirection',arrowSource=point}}
        else
            secondPoint=Vector(point)
            dir=secondPoint-firstPoint
            sim.addLog(sim.verbosity_scriptinfos,string.format("Direction: (%.3f %.3f %.3f)",unpack(dir:data())))
