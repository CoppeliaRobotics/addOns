...
function sysCall_nonSimulation()
    ...
    if not areObjectsSame(selectedObjects,previousSelectedObjects) then
        ...
        if #selectedObjects>0 then
            ...
            allShapesAndCols={}
            for i=1,#selectedObjects,1 do
                local s=sim.getObjectStringParam(selectedObjects[i],sim.shapestringparam_colorname)
                local r,a_cols=sim.getShapeColor(selectedObjects[i],'@compound',sim.colorcomponent_ambient_diffuse)
                local r,b_cols=sim.getShapeColor(selectedObjects[i],'@compound',sim.colorcomponent_specular)
                local r,c_cols=sim.getShapeColor(selectedObjects[i],'@compound',sim.colorcomponent_emission)
                local scolnms={}
                if s and s~='' then
                    local i=0
                    for token in string.gmatch(s,"[^%s]+") do
                        local col={{a_cols[3*i+1],a_cols[3*i+2],a_cols[3*i+3]},{b_cols[3*i+1],b_cols[3*i+2],b_cols[3*i+3]},{c_cols[3*i+1],c_cols[3*i+2],c_cols[3*i+3]}}
                        if token~='*' then
                            colName=token
                            allCols[colName]=col
                        else
...
function showDlg2()
    ...
    cn=cn:gsub("*","Unnamed color ")
    local xml ='<ui title="Color ['..cn..']" activate="false" closeable="true" on-close="close2_callback" layout="vbox" '..pos..'>'
    xml=xml..[[
...
function sliderMoved(ui,id,v)
    ...
    for i=1,#selectedObjects,1 do
        ...
        sim.setShapeColor(selectedObjects[i],"@compound",sim.colorcomponent_ambient_diffuse,a_cols)
