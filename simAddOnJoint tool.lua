...
function sysCall_init()
    simUI=require'simUI'
    sim.addLog(sim.verbosity_scriptinfos,"Select a model, a joint group, or individual joints to use the Joint tool.")
    sel={}
    idToJointMap={}
    jointToIdMap={}
end
...
function onSelectionChanged()
    ...
    for i,objectHandle in ipairs(sel) do
        if sim.readCustomDataBlock(objectHandle,'__jointGroup__') then
            for j,jointHandle in ipairs(sim.getReferencedHandles(objectHandle)) do
                if sim.getObjectType(jointHandle)==sim.object_joint_type then
                    ...
                    sim.addLog(sim.verbosity_warnings,'ignoring object referenced by joint group '..sim.getObjectAlias(objectHandle,2)..': '..sim.getObjectAlias(jointHandle,2))
                end
            end
        ...
        elseif sim.getObjectType(objectHandle)==sim.object_joint_type then
            ...
            sim.addLog(sim.verbosity_warnings,'ignoring selected object: '..sim.getObjectAlias(objectHandle,2))
        end
    end
