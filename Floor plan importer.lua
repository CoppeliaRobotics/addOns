...
function optimize(grid)
    local function findLargestRectFrom(grid, i, j)
        local function maxHeightWithFixedW(grid, i, j, w)
            ...
            if grid[j][i] ~= 0 then
                ...
                while jj <= #grid and grid[jj][i] ~= 0 do
                    local ii = i
                    while ii - i < w do
                        if grid[jj][ii] == 0 then return jj - j end
                        ii = ii + 1
                    end
                    jj = jj + 1
                end
        ...
        while max_w <= #grid[1] - i and grid[j][i + max_w] ~= 0 do
            ...
            if max_hh > max_h then max_h = max_hh end
...
function sysCall_init()
    ...
    colorCSS = function(c)
        return string.format(
                   'rgb(%d,%d,%d)', math.floor(255 * c[1]), math.floor(255 * c[2]),
                   math.floor(255 * c[3])
               )
    end
    ...
    addCuboid = function(classTbl, x1, y1, x2, y2)
        ...
        for j, classTbl1 in ipairs(classTbl.zSet) do
            local h = sim.createPureShape(
                          0, 1 + (respondable and 8 or 0) + 16,
                          {x2 - x1, y2 - y1, classTbl1.zMax - classTbl1.zMin}, 0
                      )
            sim.setObjectPosition(
                h, {(x1 + x2) / 2, (y1 + y2) / 2, (classTbl1.zMax + classTbl1.zMin) / 2}
            )
    ...
    addCuboidI = function(classTbl, i1, j1, i2, j2)
        ...
        return addCuboid(
                   classTbl, (i1 - 0.5) * pixelSize, (j1 - 0.5) * pixelSize, (i2 + 0.5) * pixelSize,
                   (j2 + 0.5) * pixelSize
               )
    end
    go = function()
        ...
        for className, classTbl in pairs(classes) do
            ...
            for j = 1, res[2] do
                ...
                for i = 1, res[1] do
                    ...
                    classTbl.grid[j][i] =
                        (v >= classTbl.valueMin and v <= classTbl.valueMax) and 1 or 0
                end
            end
            if optimizationEnabled then
                ...
                for i, rect in ipairs(classTbl.rects) do
                    table.insert(
                        handles, addCuboidI(
                            classTbl, rect[1] - c[1], rect[2] - c[2], rect[3] - c[1], rect[4] - c[2]
                        )
                    )
                end
            else
    ...
    if #imageFiles > 0 then
        ui = simUI.create(
            ...
            <group layout="form">
                ...
                <label text="Color:" />
                <button on-click="chooseColor" text=" " style="background-color: ]] ..
                     colorCSS(color) .. [[" id="800" />
