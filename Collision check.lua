        ...
        if sim.getObjectSpecialProperty(obj) & sim.objectspecialproperty_collidable > 0 then
            ...
            if not ignoreModelOverride then
                while obj ~= -1 do
                    ...
                    if (p & (sim.modelproperty_not_model | sim.modelproperty_not_collidable)) ==
                        sim.modelproperty_not_collidable then
                        retVal = false
                        break
                    end
...
function adjust(obj)
    ...
    while #toExplore > 0 do -- process hierarchy levels sequentially
            ...
            if sim.getModelProperty(obj) & sim.modelproperty_not_model ~= 0 or (not modelDone) then
                ...
                if couldCollide(obj, t, true) then
                    ...
                    sim.setObjectInt32Param(
                        obj, sim.objintparam_collection_self_collision_indicator, indic
                    )
                end
                local i = 0
                while true do
        ...
        if found then indic = indic + 1 end
...
function coll(ui, id)
    ...
    local colors = {
        ...
        {0, 0.5, 0}, {0, 0.25, 0.5}, {0.5, 0.5, 0}, {0.5, 0, 0.5}, {0, 0.5, 0.25},
    }
    local colInd = 1
    local function getCol(ind, b)
        ...
        colInd = colInd + 1
        if colInd > #colors then colInd = 1 end
        if b then
                ...
                if objPair[1] ~= objPair[2] then
                    ...
                    if couldCollide(objPair[2], t[2]) then
                        ...
                        if allCollisions[objPair[1]][objPair[2]] == nil then
                            local indic1 = sim.getObjectInt32Param(
                                               objPair[1],
                                               sim.objintparam_collection_self_collision_indicator
                                           )
                            local indic2 = sim.getObjectInt32Param(
                                               objPair[2],
                                               sim.objintparam_collection_self_collision_indicator
                                           )
                            if math.abs(indic1 - indic2) ~= 1 and math.abs(indic1 - indic2) ~= 10 and
                                ...
                                if r > 0 then
                                    sim.addLog(
                                        ...
                                        "collision detected between object " ..
                                            sim.getObjectAlias(objPair[1], 5) .. " and object " ..
                                            sim.getObjectAlias(objPair[2], 5)
                                    )
                                    if restoreData.dummy == -1 then
                                        restoreData.dummy = sim.createDummy(0.01)
                                        sim.setObjectInt32Param(
                                            restoreData.dummy, sim.objintparam_visibility_layer, 0
                                        )
                                        sim.setObjectProperty(
                                            restoreData.dummy, sim.objectproperty_collapsed
                                        )
                                    ...
                                    for i = 1, 2, 1 do
                                        if restoreData.origsMap[objPair[i]] == nil then
                                            ...
                                            if t[i] == sim.sceneobject_shape then
                                                ...
                                                sim.setShapeColor(
                                                    copy, nil, sim.colorcomponent_ambient_diffuse,
                                                    getCol(i)
                                                )
                                                sim.setShapeColor(
                                                    copy, nil, sim.colorcomponent_specular,
                                                    {0.1, 0.1, 0.1}
                                                )
                                                sim.setShapeColor(
                                                    copy, nil, sim.colorcomponent_emission,
                                                    {0, 0, 0}
                                                )
                                            end
                                            if t[i] == sim.sceneobject_octree then
                                                ...
                                                sim.insertVoxelsIntoOctree(
                                                    copy, 1, sim.getOctreeVoxels(objPair[i]),
                                                    getCol(i, true)
                                                )
                                            end
                                            if t[i] == sim.sceneobject_pointcloud then
                                                ...
                                                sim.insertPointsIntoPointCloud(
                                                    copy, 1, sim.getPointCloudPoints(objPair[i]),
                                                    getCol(i, true)
                                                )
                                            end
                                            ...
                                            sim.setObjectInt32Param(
                                                copy, sim.objintparam_visibility_layer, 65535
                                            )
    ...
    if restoreData.dummy == -1 then
        ...
        if id == 2 then restoreData.selfCollEntity = obj1 end
...
function sysCall_init()
    ...
    sim.addLog(
        sim.verbosity_scriptinfos,
        "This add-on allows to quickly verify the collision state of one or two entities. Just select one or two entities."
    )
    obj1 = -1
    obj2 = -1
end
...
function showDlg()
    if not ui and sim.isHandle(obj1) then
        ...
        if obj2 == -1 then
            if obj1IsModel then
                txt1 = "Check for collision with the environment for model " ..
                           sim.getObjectAlias(obj1, 5)
                txt2 = "Check for self-collision for model " .. sim.getObjectAlias(obj1, 5)
            else
                txt1 = "Check for collision with the environment for object " ..
                           sim.getObjectAlias(obj1, 5)
            end
        else
        ...
        local xml =
            '<ui title="Collision check" activate="false" closeable="true" on-close="close_callback" layout="vbox" ' ..
                pos .. '>'
        xml = xml .. '<button text="' .. txt1 ..
                  '" on-click="coll" style="* {min-width: 300px; min-height: 50px;}" id="1"/>'
        if txt2 then
...
function restoreScene(selfCollEntity)
    if restoreData then
        ...
        if selfCollEntity and sim.isHandle(selfCollEntity) then
            ...
            if simUI.msgBox(
                simUI.msgbox_type.question, simUI.msgbox_buttons.yesno, "Model self-collision", txt
            ) == simUI.msgbox_result.yes then adjust(selfCollEntity) end
        end
    end
end
